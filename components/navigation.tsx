          ...
          <div className="hidden md:flex space-x-12">
            {navItems.map((item) => (
              <button
                ...
                className={`text-sm font-light transition-colors hover:text-foreground ${
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-background border-t border-border/50">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`block px-3 py-2 text-base font-light transition-colors hover:text-foreground ${
                    activeSection === item.id ? "text-foreground" : "text-muted-foreground"
                  }`}
                >
                  {item.label}
